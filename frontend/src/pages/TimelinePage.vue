...
import { DEMO_SCENARIO_ID, DEMO_BRANCH } from '../demoData'
...
function onNodeClick(event: any) {
  ...
  if (nodeId && nodeId !== 'ghost-loading') {
    selectNode(nodeId)
  }
}
...
async function onForkSubmit(whatif: string) {
  ...
  if (isDemo.value && !demoForked) {
    ...
    store.addBranch(popoverNodeId.value, DEMO_BRANCH, store.nextBranchColor)
...
<template>
  <div class="timeline-page">
    ...
    <div v-if="store.isForking" class="fork-loading">
      <div class="fork-loading-text">Exploring alternate timeline…</div>
    </div>
